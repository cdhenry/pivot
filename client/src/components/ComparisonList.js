      ...
      <table className="TableShowStyle table-hover">
        ...
        <tbody>
          <tr className="table-primary">
            ...
            {comparison.predictors.map((predictor) =>
              <th scope="col">{predictor.engagement}</th>)}
          </tr>
          <tr className="table-light">
            <th scope="row">Work that helps others</th>
            {comparison.predictors.map((predictor) =>
              <th scope="col">{predictor.altruism}</th>)}
          </tr>
          <tr className="table-primary">
            <th scope="row">Being good at your job</th>
            {comparison.predictors.map((predictor) =>
              <th scope="col">{predictor.skill}</th>)}
          </tr>
          <tr className="table-light">
            <th scope="row">Supportive Colleagues</th>
            {comparison.predictors.map((predictor) =>
              <th scope="col">{predictor.support}</th>)}
          </tr>
          <tr className="table-primary">
            <th scope="row">Basic Needs</th>
            {comparison.predictors.map((predictor) =>
              <th scope="col">{predictor.basic_needs}</th>)}
          </tr>
          <tr className="table-light">
            <th scope="row">Fit with rest of life</th>
            {comparison.predictors.map((predictor) =>
              <th scope="col">{predictor.balance}</th>)}
          </tr>
        </tbody>
      </table>
      <br/>
